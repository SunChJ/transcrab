---
title: Claude 来值班：Claude Tag 如何成为 Anthropic CI/CD 故障的第一响应者
...
* 告诉我们是否需要对 Kubernetes 集群的某些部分执行 drain（驱逐工作负载）或 cordon（标记为不可调度）。
...
坦白说，报告格式经过了好几轮迭代。Claude 可以一次就写出一个生成状态报告的 skill，但怎样才算易读，取决于团队自己的偏好。这是人与人之间的沟通问题，不是把系统接通就行。
...
如今，我们的软件工程师平均每季度交付的代码量，是 2021 至 2025 年期间的 [8 倍](https://www.anthropic.com/institute/recursive-self-improvement)。我们一直维持着高质量标准：每个 PR 都有明确的人类负责人，每次变更都必须获得批准才能合并，每次变更都要经过同一套 CI 检查关卡。而要跟上智能体编程的步伐，唯一的办法就是让 CI 也由智能体驱动。
