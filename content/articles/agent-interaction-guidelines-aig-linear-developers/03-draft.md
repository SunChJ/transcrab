...
Agent 正在改变软件被规划、构建、审查与部署的方式。由于 agent 能大规模地产出工作，角色分工与工作流也会被重塑。价值的重心，开始转向输入编排、上下文工程，以及对输出结果的审阅。
