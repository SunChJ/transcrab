---
title: 隆重推出 Kitesurf：在 Cloudflare Workers 的 V8 隔离实例中运行、智能体优先的浏览器
date: '2026-08-07T06:55:35.309Z'
...
## Kitesurf 已通过超过 215,000 项 WPT 测试，而且还在增长

Kitesurf 确实能用。它已经通过超过 215,000 项 [WPT 测试](https://github.com/web-platform-tests/wpt)，而且每周还会新增数百项通过的测试。下图展示了从项目启动至今，通过测试数随版本演进的情况：
...
Chromium 的 CPU 用量是 Kitesurf 的 3.1 倍
...
Chromium 的 CPU 用量是 Kitesurf 的 3.8 倍
...
Chromium 的内存用量是 Kitesurf 的 4.7 倍
...
Chromium 的内存用量是 Kitesurf 的 7.0 倍
...
但在真正决定账单的内存与 CPU 方面，Kitesurf 胜过 Chromium，其用量仅为后者的约七分之一至三分之一。更少的内存意味着我们可以运行更多会话、更好地扩展，并从根本上降低我们和你的成本。
