---
title: 发布 Cloudflare Wallets：面向智能体互联网的可编程钱包
date: '2026-08-05T01:50:41.585Z'
...
Cloudflare Wallets 将让你能够存储稳定币、购买服务，并在整个 Web 上接收资金。每个拥有钱包的账户还可以为其智能体创建 Virtual Wallets，让它们购买 API、MCP Tools、内容等。你可以为 Virtual Wallets 设置约束规则（例如额度、允许列表和单笔交易上限），帮助智能体安全地使用你账户中的资金。这样一来，智能体便能以较低门槛、在风险可控的前提下试用众多 API。钱包用户也可以选择分享自己的 Cloudflare Wallet 钱包标识，在与商家互动时获得稳定身份。
...
当你或你的智能体选定要使用的 API 后，你在 Account Wallet 中设置的策略将成为 Virtual Wallets 的成本控制措施。想给每位员工每周 100 美元的 AI 推理预算？只需为 Account Wallet 配置适当的余额，再为每位员工创建一个应用该规则的 Virtual Wallet。任何超出其 Virtual Wallet 限额的人，都可以向有权修改 Account Wallet 的人员申请人工例外批准。
