...
我们检查了 Pi 的 78 个官方扩展示例。其中 60 个无状态；17 个有状态的示例里，只有两个是正确的。
...
从设计边界里的 *Factorio* 场景出发。假设我们克隆 roboomp，让 gpt spark 把所有名字替换成 CodeWhatever，然后凭这套神奇技术向用户收取数千元费用。工具由谁运行？当然是 VM。嗯，真这么简单吗？
...
图像还可能以 base64 文本混在某一行里，这就更糟了。仅检查一行是否为图像行的 `.includes`，就占了整个会话 CPU 周期的 20%。账单可不小——而且这个会话根本没有图像！
